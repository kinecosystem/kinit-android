...
class TaskRepository(category: Category) {
    ...
    private val chosenAnswersCache: DataStore = dataStoreProvider.dataStore(category.id + QUESTIONNAIRE_ANSWERS_STORAGE)
    private val taskCache: DataStore = dataStoreProvider.dataStore(category?.id + TASK_STORAGE)
