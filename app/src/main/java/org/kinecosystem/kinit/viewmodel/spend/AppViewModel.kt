...
class AppViewModel(private val navigator: Navigator, private val app: EcoApplication) : ViewPager.OnPageChangeListener {
    ...
    fun onActionBtnClicked(view: View?) {
        if (canTransferKin) {
            analytics.logEvent(Events.Analytics.ClickSendButtonOnAppPage(app.data.categoryTitle, app.identifier, appName, true))
        } else {
            analytics.logEvent(Events.Analytics.ClickGetButtonOnAppPage(app.data.categoryTitle, app.identifier, appName, false))
